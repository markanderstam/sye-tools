...
async function deleteResourceGroup(ctx: Context) {
    try {
        ...
        consoleLog('  Deleting...')
        await exec('az', ['group', 'delete', ...ctx.subscriptionArgs, '--resource-group', ctx.resourceGroup, '--yes'])
        consoleLog('  Done.')
    } catch (ex) {
...
async function deleteServicePrincipal(ctx: Context) {
    try {
        ...
        consoleLog('  Deleting...')
        await exec('az', ['ad', 'sp', 'delete', ...ctx.subscriptionArgs, '--id', ctx.servicePrincipalHomePage])
        consoleLog('  Done.')
    } catch (ex) {
        consoleLog('  Already deleted - OK.')
    }
