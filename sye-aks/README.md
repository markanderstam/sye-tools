...
`sye aks cluster-create` automatically opens UDP port `2123` to `2130` and TCP `2505` towards all worker nodes for this purpose.

If the flag `--open-ssh-port` is given to the `sye aks cluster-create` command the SSH port (TCP/22) will be opened as well. 
