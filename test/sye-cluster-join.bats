...
@test "imageReleaseRevision should get latest tag from local registry" {
    ...
    local curl_args_2="-s -k -u user:pass https://dockerregistry.neti.systems:5000/v2/ott/release/manifests/r29.1"
    stub curl \
        "${curl_args_1} : echo '${release_manifest}'" \
        "${curl_args_2} : echo '${release_manifest}'"
    ...
    run imageReleaseRevision "${repository_url}" "user" "pass" "${service}" "r29.1"
    [ "$status" -eq 0 ]
    [ "$output" = "${service_version}" ]
